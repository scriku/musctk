...
SUBROUTINE expand_g_v_f()
  ...
  !
  USE sctk_val, ONLY : fbee, lbee, nqbz, Vc0, VcF, lsf, nci
  USE sctk_cnt_dsp, ONLY : cnt_and_dsp
  !
  ...
  REAL(dp),ALLOCATABLE :: Vc1(:,:,:,:,:), wght(:,:), wght0(:,:)
  !
...
SUBROUTINE k_kplusq_sym_f(nindmax,nind,ind)
  ...
  WRITE(stdout,'(7x,"nq1 : ",i0)') nq1
  WRITE(stdout,'(7x,"nq2 : ",i0)') nq2
  WRITE(stdout,'(7x,"nq3 : ",i0)') nq3
  !
  DO ik = 1, nqbz
     !
     ikv1(1) = (ik - 1) / (nq2*nq3)
