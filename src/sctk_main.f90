...
PROGRAM sctk_main
  ...
  USE sctk_read_file, ONLY : read_elph, read_Coulomb, read_a2Fsave, degenerated_band, read_qgrid
  USE sctk_rotate_kernel, ONLY : expand_g_v, expand_g_v_f
  USE sctk_ini_delta, ONLY : ini_delta, ini_lambda_mu, energy_grid, get_nbnd
  ...
  ELSE ! calculation = scdft, lambda_mu_k, qpdos, deltaf, ultrasonic
     ...
     ELSE ! calculation = lambda_mu_k, qpdos, deltaf, ultrasonic
        ...
        IF(TRIM(calculation) == "lambda_mu_k") THEN
           ...
           !
           WRITE(stdout,'(/,5x,"#####  Write mu.frmsf  #####",/)')
           IF(mpime == 0) THEN
