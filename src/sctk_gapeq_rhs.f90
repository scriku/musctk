...
SUBROUTINE make_lambda_mu_f()
  ...
  USE io_global, ONLY : stdout
  !
  USE sctk_val, ONLY : bindx, dk, kindx, ngap, dltf, VcF, nci, freq_min, el_nbnd_min, el_nbnd_max
  USE sctk_gauss_legendre, ONLY : weightspoints_gl
  ...
  REAL(dp) :: mu, weight, dosd(el_nbnd_min:el_nbnd_max,nks)
  !
  CALL start_clock("make_lambda_mu_f")
  !
  ALLOCATE(dltf(1,b_low:b_high,nks)) !dltf -> mu_k
