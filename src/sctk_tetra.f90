...
SUBROUTINE calc_dosk(dosd)
  ...
  USE ener, ONLY : ef
  !
  USE sctk_val, ONLY : nx, xi0, el_nbnd_min, el_nbnd_max
  !
  IMPLICIT NONE
  !
  REAL(dp),INTENT(OUT) :: dosd(nx,el_nbnd_min:el_nbnd_max,nks)
  !
  INTEGER :: ibnd, it, ii, ix, itetra(4), ntetra0, ntetra1
  REAL(dp) :: ei(4,el_nbnd_min:el_nbnd_max), e(4), a(4,4), w1(nx,4), V
  !
  dosd(1:nx, el_nbnd_min:el_nbnd_max, 1:nks) = 0.0_dp
  !
  ...
  DO it = ntetra0, ntetra1
     !
     DO ibnd = el_nbnd_min, el_nbnd_max
  ...
  !$OMP END PARALLEL
  !
  dosd(  1:nx, el_nbnd_min:el_nbnd_max, 1:nks) = &
  & dosd(1:nx, el_nbnd_min:el_nbnd_max, 1:nks) / REAL(ntetra, dp)
  !
